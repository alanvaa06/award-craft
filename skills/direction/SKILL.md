...
3. **Calibration dials** — three 1-10 values inferred from the brief, each with a
...
4. **Art direction** — medium, lighting recipe, grade, texture, mood; explicit
   anti-references (what this must NOT look like).
5. **Wireframe** — ASCII wireframe per script section, in script order. If no
   script: preloader → hero → 2-4 body chapters → interactive footer
   (anatomy in awwwards-rubric.md).
6. **Signature moment** — ONE unique, memorable device, tied to the script's
...
7. **Drift vs brand source** — see below. Empty section allowed only when no
   brand source exists.
8. **Asset slots** — every media slot with ratio + treatment — use ONLY the
